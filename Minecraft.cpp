...
Sparrow::minecraftVersion::minecraftVersion(const QString &version, const enum versionType &type,
                                            const fileInfo &assetIndex, const QString &assetVersion,
                                            const fileInfo &clientJar, const QVector<fileInfo> &libs,
                                            const QString &mainClass) : version(version), type(type),
                                                                        assetIndex(assetIndex),
                                                                        assetVersion(assetVersion),
                                                                        clientJar(clientJar), libs(libs),
                                                                        mainClass(mainClass) {
...
Sparrow::Minecraft::Minecraft(const minecraftVersion &version, const MinecraftDirectory &root, const QDir &path,
                              const QString &clientJarFile, const QString &jsonFile) : version(version), root(root),
                                                                                       path(path),
